...
const Banner = () => {
  ...
  const getMovie = async () => {
    try {
      const res = await axios.get(
        `https://api.themoviedb.org/3/movie/${params?.id}?api_key=${APIKEY}&language${lang}-US`
      );
  ...
  const getCredits = async () => {
    try {
      const res = await axios.get(
        `https://api.themoviedb.org/3/movie/${params?.id}/credits?api_key=${APIKEY}&language${lang}-US`
      );
